...
const { title, description, status, priority, deadline, projectId } = result.data;
    ...
    if(!existingTask || existingTask.userId !== payload.userId){return NextResponse.json({ error: "Задача не найдена" }, { status: 404 });
  }
    ...
    if(status !== undefined){updateData.status = status;
      if (status === "IN_PROGRESS" && !existingTask.startedAt){updateData.startedAt = new Date();}
      if (status === "DONE"){updateData.completedAt = new Date();
        if(!existingTask.startedAt){updateData.startedAt = new Date();}}
      if (status !== "DONE" && existingTask.completedAt){updateData.completedAt = null;}
    }
    const task = await prisma.task.update({ where: { id }, data: updateData, include: { project: true }, });
